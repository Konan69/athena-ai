  ...
  return (
    <ChatInputContext.Provider value={contextValue}>
      <div
        className={cn(
          variant === "default" &&
            "flex flex-col items-end w-full p-2 rounded-t-lg border border-input/60 bg-background focus-within:outline-none",
...
function ChatInputSubmit({
  ...
  if (loading && onStop) {
    return (
      <Button
        onClick={onStop}
        className={cn(
          "shrink-0 rounded-lg p-1.5 h-10 border dark:border-zinc-600",
          className
        )}
        {...props}
      >
  ...
  return (
    <Button
      className={cn(
        "shrink-0 rounded-lg p-1.5 h-10 border dark:border-zinc-600 mt-1.5",
        className
      )}
      disabled={isDisabled}
      onClick={(event) => {
